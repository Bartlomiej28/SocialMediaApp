  ...
  return (
    <section className='flex justify-start items-center flex-col h-screen'>
      <div className='relative w-full h-full'>
        <video src={tiktokVideo} type='video/mp4' loop controls={false} muted autoPlay className='w-full h-full object-cover'/>
        <div className='absolute flex flex-col justify-center items-center top-0 right-0 left-0 bottom-0 bg-blackOverlay'>
          <div className='shadow-2xl w-auto h-auto flex flex-row gap-8'>
            <button data-test="sign-in-button" className='border bg-white px-4 py-2 rounded-3xl' onClick={handleSignInWindow}>Zaloguj się</button>
            <button data-test="sign-up-button" className='border bg-white px-4 py-2 rounded-3xl' onClick={handleSignUpWindow}>Zarejestruj się</button>
          </div>
        </div>
