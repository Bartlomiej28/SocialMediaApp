...
function TikTok({ publishedBy, content, filePath, tiktokId, likedBy, comments }) {
    ...
    const handleGoToProfile = () => {
        if (publishedBy === currentUser) {
            ...
            navigate(`/user/${publishedBy}`);
        }
    };

    const handleSetWholeText = () => {
        setShowWholeText((prev) => !prev);
    };
            ...
            {windowWidth > 640 ? ( 
                <div className='relative w-1/2 xl:w-8/12 lg:w-9/12 md:w-8/12 flex flex-col shadow h-sidebar-height min-h-999 snap-start'>
                    <div className='flex flex-row w-full h-1/6 pt-2 transparent p-4'>
                        <div className='h-auto w-auto mr-2'>
                            <img src={authorData.photoURL} onClick={handleGoToProfile} width={60} alt='profile' className='rounded-full cursor-pointer' />
                        </div>
                    ...
                    <div className='flex flex-row h-5/6'>
                        ...
                        <div className='w-1/6 h-full flex flex-col justify-end items-center p-2'>
                            <div className='absolute bottom-20'>
                                <span>
                                    <button
                                        className={`bg-[#f1f1f2] md:bg-transparent flex items-center justify-center h-12 w-12 rounded-full duration-300 ${isLiked ? 'text-red-500' : 'text-black'}`}
                                        onClick={handleLikeAndAddOneLike}
                                    >
                                        <BsHeartFill fontSize={20} />
                                    </button>
                                    <label className='flex justify-center text-black'>{likes}</label>
                                </span>
                                <span>
                                    <button
                                        className='bg-[#f1f1f2] md:bg-transparent flex items-center justify-center h-12 w-12 rounded-full text-black'
                                        onClick={handleCommentsSection}
                                    >
                                        <FaCommentDots fontSize={20} />
                                    </button>
                                    <label className='flex justify-center text-black'>{comments.length}</label>
                                </span>
                                <span>
                                    <button
                                        className='bg-[#f1f1f2] md:bg-transparent flex items-center justify-center h-12 w-12 rounded-full focus:bg-red-500 duration-300 text-black'
                                        onClick={handleShare}
                                    >
                                        <FaShare fontSize={20} />
                                    </button>
                ...
                <div className='relative w-full sm:w-full flex flex-col bg-white sm:bg-black h-screen min-h-999 snap-start'>
                    <div className={`flex flex-col w-10/12 p-4 absolute z-10 bottom-20 min-h-[16.666667%] h-1/6 ${showWholeText ? 'h-auto' : ''}`}>
                        <p onClick={handleGoToProfile} className='cursor-pointer text-white font-bold'>
                            {authorData.displayName}
                        </p>
                        <p
                            className={`text-white truncate text-wrap focus:bg-black ${showWholeText ? 'h-auto bg-black bg-opacity-50 rounded-sm transition delay-100' : ''}`}
                            onClick={handleSetWholeText}
                        >
                            {correctContent}
                        </p>
                    </div>
                    ...
                    <div className='absolute bottom-28 right-0 w-min h-min flex flex-col justify-end items-center p-4'>
                        <div className='h-auto mb-4'>
                            <img src={authorData.photoURL} onClick={handleGoToProfile} alt='profile' width={60} className='rounded-full cursor-pointer border border-[#fe2c55]' />
                        </div>
                        <span className='-mb-2'>
                            <button
                                className={`flex items-center justify-center h-12 w-12 rounded-full duration-300 ${isLiked ? 'text-red-500' : 'text-white'}`}
                                onClick={handleLikeAndAddOneLike}
                            >
                        ...
                        <span className='-mb-2'>
                            <button className='flex items-center justify-center h-12 w-12 rounded-full text-white' onClick={handleCommentsSection}>
                        ...
                        <span>
                            <button
                                className='bg-[#f1f1f2] md:bg-transparent flex items-center justify-center h-12 w-12 rounded-full focus:bg-red-500 duration-300 text-black md:text-white'
                                onClick={handleShare}
                            >
                                <FaShare fontSize={30} />
                            </button>
